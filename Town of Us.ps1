cmd.exe /c curl https://api.github.com/repos/eDonnes124/Town-Of-Us-R/releases/latest | findstr "browser_download_url" >tmp
$targetFolder = ".\Town of Us\"
mkdir $targetFolder
$url = get-content tmp
$url = [regex]::Matches($url, 'https:[\/\w\.\d-]+zip').Value
cmd.exe /c curl -Lo temp.zip $url[1]
Expand-archive .\temp.zip -destinationpath $targetFolder
cd $targetFolder
$folderName=Get-childitem -path .\ | select-object -expandProperty Name
xcopy /i /s /c $folderName .\
cd..
